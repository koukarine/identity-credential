    ...
    try {
        ...
        if (documentType.mdocDocumentType != null) {
            ...
            if (algorithm != Algorithm.UNSET) {
                ...
                val readerAccess = buildCborArray {
                    add((App.getInstance().bundledReaderRootKey.publicKey as EcPublicKeyDoubleCoordinate)
                        .asUncompressedPointEncoding)
                    add((App.getInstance().readerKeyForBackend as EcPublicKeyDoubleCoordinate)
                        .asUncompressedPointEncoding)
                    add((App.getInstance().readerKeyForUntrustedDevices as EcPublicKeyDoubleCoordinate)
                        .asUncompressedPointEncoding)
                }
                ...
                val unprotectedHeaders = mapOf<CoseLabel, DataItem>(
                    Pair(
                        ...
                        X509CertChain(dsCert.certificates).toDataItem()
                    )
                )
