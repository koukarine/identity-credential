...
fun Camera.enableFaceDetection(
    ...
    private fun processFaces(faces: List<Face>, imageProxy: ImageProxy, imageSize: Size, imageRotation: Int) {
        if (faces.isNotEmpty()) {
            if (faces.size == 1) {
                ...
                Logger.d(
                    TAG,
                    "Left Eye Position: $leftEyePosition, Right Eye Position: $rightEyePosition, Mouth Position: $mouthPosition"
                )
...
actual fun platformStartDetection(
    ...
    camera.enableFaceDetection(onFaceDetected, onImageSize, onImageRotation, coroutineScope)
}